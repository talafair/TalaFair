  ...
  <form method="POST" action="{{ route('users.update', $user) }}">
    ...
    <div class="card yg-card mb-4">
      <div class="card-body p-4">
        ...
        <div class="row g-3">
          <div class="col-md-4"><label for="role" class="form-label">Category</label><select name="role" id="role" class="form-select" required>@foreach (\App\Models\User::CATEGORIES as $value => $label)<option value="{{ $value }}" @selected($user->role === $value)>{{ $label }}</option>@endforeach</select></div>
            <div class="col-md-4"><label for="role" class="form-label">Category</label><select name="role" id="role" class="form-select" required>@foreach (\App\Models\User::CATEGORIES as $value => $label)<option value="{{ $value }}" @selected(($user->role ?? 'resident') === $value)>{{ $label }}</option>@endforeach</select></div>
    ...
    <div class="card yg-card mb-4">
      <div class="card-body p-4">
        ...
        <div class="row g-3">
          <div class="col-md-6"><span class="form-label d-block">Gender</span><div class="d-flex flex-wrap gap-3">@foreach (['female' => 'Female', 'male' => 'Male', 'others' => 'Others'] as $value => $label)<div class="form-check"><input class="form-check-input" type="radio" name="gender" id="gender_{{ $value }}" value="{{ $value }}" required @checked($user->gender === $value)><label class="form-check-label" for="gender_{{ $value }}">{{ $label }}</label></div>@endforeach</div><input name="gender_other" value="{{ $user->gender_other }}" class="form-control mt-2" data-title-case placeholder="Please specify if needed"></div>
          ...
          <div class="col-md-6"><label for="occupation" class="form-label">Occupation <span class="text-secondary fw-normal">(optional)</span></label><input id="occupation" name="occupation" value="{{ $user->occupation }}" class="form-control" data-title-case></div>
          <div class="col-md-4"><input type="hidden" name="is_student" value="0"><div class="form-check mt-md-4"><input class="form-check-input" type="checkbox" name="is_student" value="1" id="is_student" @checked($user->is_student)><label class="form-check-label" for="is_student">Is a student</label></div></div>
          <div class="col-md-8"><label for="school" class="form-label">Name of school</label><input id="school" name="school" value="{{ $user->school }}" class="form-control" data-title-case></div>
        </div>
      </div>
    </div>
