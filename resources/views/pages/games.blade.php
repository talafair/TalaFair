...
(() => {
    ...
    function renderRankings(element, rankings, scoreKey = 'score') { const target = $(element); target.innerHTML = rankings.length ? rankings.map((item, index) => `<tr><td><span class="badge ${index < 3 ? 'badge-gold' : 'badge-soft'} rounded-pill">#${index + 1}</span></td><td class="fw-semibold">${item.name}</td><td>${item[scoreKey]} pts</td><td class="text-secondary">${item.played_at ?? ''}</td></tr>`).join('') : '<tr><td colspan="4" class="text-secondary">No scores yet.</td></tr>'; }
    ...
    async function submitRun(game, scores) { const response = await fetch(`/games/${game}/finish?period=${rankingPeriod}`, { method: 'POST', headers: {'Content-Type':'application/json','X-CSRF-TOKEN':csrf,'Accept':'application/json'}, body: JSON.stringify({stage_scores:scores}) }); return response.ok ? response.json() : null; }
    ...
    function finishMemoryStage() { if (memoryEnded) return; memoryEnded = true; clearInterval(memoryTimer); memoryScores.push(Math.max(0, memoryValues.length * 2 - memoryTurns)); memoryStage++; if (memoryStage < 3) setTimeout(runMemoryStage, 500); else submitRun('memory-test', memoryScores).then(result => { const total = result?.total_score ?? (memoryScores[0] + memoryScores[1] * 2 + memoryScores[2] * 3); summary(memoryScores, total, $('memoryBoard')); $('memoryFinalScore').textContent = `Final Score: ${total} points`; renderRankings('memoryRankings', result?.rankings ?? []); $('memoryStart').disabled = true; $('memoryStart').innerHTML = '<i class="bi bi-check-circle me-1"></i>Played today'; }); }
